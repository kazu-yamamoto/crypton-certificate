...
tryExtRawASN1 (ExtensionRaw oid _ content) =
    case decodeASN1' BER content of
        Left err ->
            Left $
                "fromASN1: X509.ExtensionRaw: OID="
                    ++ show oid
                    ++ ": cannot decode data: "
                    ++ show err
        Right r -> Right r
...
instance ASN1Object Extensions where
    ...
    fromASN1 s = runParseASN1State (Extensions <$> parseExtensions) s
      where
        parseExtensions = onNextContainerMaybe Sequence (getMany getObject)
...
instance ASN1Object ExtensionRaw where
    ...
    fromASN1 (Start Sequence : OID oid : xs) =
        case xs of
            ...
            _ ->
                Left ("fromASN1: X509.ExtensionRaw: unknown format:" ++ show xs)
    fromASN1 l =
...
encodeExt (ExtensionRaw oid critical content) =
    asn1Container
        Sequence
        ([OID oid] ++ (if critical then [Boolean True] else []) ++ [OctetString content])
